...
import { requestGuidance, checkGuidanceStatus } from '@/lib/services/ai-guidance-service';
...
export function useAiGuidance() {
  ...
  const fetchGuidance = async (jobDescription: string, experience: string, userId: string, pitchId?: string) => {
    console.log("[useAiGuidance] fetchGuidance called - setting isLoading to true");
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      if (!result.isSuccess) {
        console.log("[useAiGuidance] fetchGuidance received error:", result.message);
        throw new Error(result.message);
      }
      
      console.log("[useAiGuidance] fetchGuidance succeeded, setting requestId:", result.data);
        ...
        checkGuidanceStatus(result.data)
          .then(statusResult => {
            if (statusResult.isSuccess && statusResult.data) {
              console.log("[useAiGuidance] Found guidance immediately:", statusResult.data.substring(0, 20) + "...");
              setGuidance(statusResult.data);
              setIsLoading(false);
            } else {
              console.log("[useAiGuidance] No immediate guidance found, will poll");
            }
          })
          .catch(err => console.error("[useAiGuidance] Error checking immediate guidance:", err));
      }, 100);
    ...
    const checkStatus = async () => {
      ...
      try {
        ...
        if (result.isSuccess && result.data) {
          console.log("[useAiGuidance] Received successful guidance data from service:", result.data);
          if (isPolling) { // Check flag before updating state
            setGuidance(result.data);
            console.log("[useAiGuidance] Setting isLoading to false");
            setIsLoading(false);
            console.log("[useAiGuidance] Current states - isLoading:", false, "guidance:", result.data?.substring(0, 20) + "...");
        ...
        attempts++;
        console.log(`[useAiGuidance] Poll attempt ${attempts}/${maxAttempts}, continuing polling`);
        if (attempts >= maxAttempts) {
          console.log("[useAiGuidance] Reached max attempts, setting error and stopping polling");
          if (isPolling) { // Check flag before updating state
        ...
        if (isPolling) { // Check flag before updating state
          setError(err instanceof Error ? err.message : 'Failed to check guidance status');
          setIsLoading(false);
          isPolling = false; // Stop polling
        }
