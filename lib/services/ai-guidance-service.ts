...
export async function requestGuidance(request: GuidanceRequest): Promise<ActionState<string>> {
  try {
...
export async function checkGuidanceStatus(requestId: string): Promise<ActionState<string>> {
  try {
    ...
    return {
      isSuccess: false,
      message: error instanceof Error ? error.message : 'Failed to check guidance status'
    };
  }
}