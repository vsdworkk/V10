...
const starTypes = require('../dist/types/star-types');
const { isString, parseLegacySituation, parseLegacyTask, parseLegacyResult } = starTypes;
...
function simulateBuildPitchPrompt(starExample: any): string {
  ...
  if (starExample) {
    ...
    if (typeof starExample.situation === 'object') {
      ...
      const situation = starExample.situation as StarSchema['situation'];
      const situationText = [
      ...
      const task = starExample.task as StarSchema['task'];
      const taskText = [
      ...
      const result = starExample.result as StarSchema['result'];
      const resultText = [
